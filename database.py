...
class Database:
	...
	def get_qid_by_loghash(self, loghash):
	...
	def get_references_by_loghash(self, loghash):
		qid = self.get_qid_by_loghash(loghash)
		sql = "SELECT * FROM article_reference WHERE id_query = '%d'" % (qid)
		self.cur.execute(sql)
		self.conn.commit()
		articles = []
		for row in self.cur.fetchall():
